...
uint32_t calculate_next_step(acceleration)
{
	static uint32_t c=0;
	...
	if (c==0) // Init
	{
		c=STEPPER_DRIVER_FREQUENCY*sqrt((2*MOTOR_STEP_ANGLE)/(acceleration));
		c_prev=c;
...
void limit_motor_speed(uint32_t time_to_next_step, int8_t direction, float acceleration)
{
	...
	{
		time_to_next_step=MAX_MOTOR_DELAY; // point of 0 speed reached so its time to start accelerating in opossite direction
		direction=-direction;
